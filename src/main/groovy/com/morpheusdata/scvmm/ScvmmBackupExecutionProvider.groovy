...
class ScvmmBackupExecutionProvider implements BackupExecutionProvider {
    ...
    ServiceResponse deleteBackupResult(BackupResult backupResult, Map opts) {
        ...
        try {
            ...
            cloud = cloud ?: (server ? morpheusContext.services.cloud.get(server.cloud.id) : null)
            if (snapshotId && server && cloud) {
                def node = provisionProvider.pickScvmmController(cloud)
                def zoneHyperVOpts = apiService.getScvmmZoneAndHypervisorOpts(morpheusContext, cloud, node)
                def result = apiService.deleteSnapshot(zoneHyperVOpts, server.externalId, snapshotId)
                if (result.success) {
                    log.debug "Snapshot deleted successfully: ${snapshotId}"
                    rtn.success = true
                } else {
     ...
     * @return a {@link ServiceResponse} indicating the success or failure of the execution preperation.
     * A success value
     ...
     * the backup execution including the status and a reference to the output that can be used in any future
     * operations.
     * @param backupResultModel the backup result instance to prepare before execution
     * @param opts additional option values influencing preparation
...
// Main method refactored
    ...
    ServiceResponse<BackupExecutionResponse> executeBackup(Backup backup, BackupResult backupResult,
        ...
        try {
            ...
            rtn.data.backupResult.backupSetId =
                    executionConfig.backupResultId ?: BackupResultUtility.generateBackupResultSetId()
    ...
    /**
     * Periodically call until the backup execution has successfully completed. The default refresh interval
     * is 60 seconds.
     * @param backupResult the reference to the results of the backup execution including the last known status.
     * Set the status to a canceled/succeeded/failed value from one of the {@link BackupStatusUtility} values
