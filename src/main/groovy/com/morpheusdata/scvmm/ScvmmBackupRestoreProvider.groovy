...
class ScvmmBackupRestoreProvider implements BackupRestoreProvider {
    ...
    /**
     * Add additional configurations to a backup restore. Morpheus will handle all basic configuration details,
     * this is a
     ...
     * @param backupResultModel backup result to be restored
     * @param instanceModel the instance the backup was created from, if it still exists. Retained backups
     * will not have a reference to the instance.
     ...
     */
    @Override
    ServiceResponse getBackupRestoreInstanceConfig(BackupResult backupResult,
                                                   Instance instanceModel, Map restoreConfig, Map opts) {
 ...
 */
     ...
     * the restore while the restore has a status of `START_REQUESTED` or `IN_PROGRESS`.
     * Any other status will indicate
     * the restore has completed and does not need to be refreshed.
     * The primary use case for this method is long-running
